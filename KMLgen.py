...
# This should be generated by DIRAC system at some point
data_sites = read_csv("input/sites.csv")
data_summary = read_csv("info/site-summary.csv")
...
for line in data_sites:
    ...
    if i != 0:
        ...
        for site in data_summary:
            if data_sites[i][0] == data_summary[j][0]:
                coincidence = True
                kml_document.append(add_kml_site(data_sites[i][0],data_sites[i][1],data_sites[i][2], \
                                    int(data_summary[j][1]),int(data_summary[j][2])))
                pie_plot(data_sites[i][0], int(data_summary[j][1]), int(data_summary[j][2]))
            j += 1
        if not coincidence:
            print "[WARNING] No summary information for " + data_sites[i][0]
            kml_document.append(add_kml_site(data_sites[i][0],data_sites[i][1], data_sites[i][2], 0, 0))
            pie_plot(data_sites[i][0], 0, 0)
    i += 1

kml_gen = open("content/KMLGen.kml","w")
